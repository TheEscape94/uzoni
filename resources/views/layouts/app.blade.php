                ...
                <div class="col-xs-12">
                    ...
                    <div class="pull-right profile">
                        ...
                        <div class="profile-notifications profile-notifications-color">
                            @if(Auth::check())
                                <span id="notification-trigger" class="numOfMsg">0</span>
                            <img class="arrow-p" src="/img/template-icons/notification-arrow-green.png">
                            @endif
            ...
            <div class="row">
                <div class="col-xs-12">
                    <ul>
                        @foreach($mainCategories as $mainCategory)
                            @if(\Illuminate\Support\Facades\App::getLocale() == 'en')
                                <li><a id="cat_{{$mainCategory->id}}" onclick="openSubC('{{$mainCategory->id}}')">{{ $mainCategory->name_en }}</a></li>
<!--                        href="/{{\Illuminate\Support\Facades\App::getLocale()}}/pretraga-po-kategorijama/{{$mainCategory->id}}"-->
                            @else
                                <li><a id="cat_{{$mainCategory->id}}"  onclick="openSubC('{{$mainCategory->id}}')">{{ $mainCategory->name_rs }}</a></li>
<!--                        href="/{{\Illuminate\Support\Facades\App::getLocale()}}/pretraga-po-kategorijama/{{$mainCategory->id}}" onmouseover="openSubC('{{$mainCategory->id}}')"-->
                            @endif
                        @endforeach
                    </ul>
                </div>
    ...
    <section class="pull-left full-width footer-subnavigation">
        <div class="container-big">
            <div class="row">
                <div class="col-xs-12">
                    @foreach($mainCategories as $mainCategory)
                        <ul id="subc_{{$mainCategory->id}}" class="subcategories-footer">
                            @foreach($mainCategory->subGroups as $subGroup)
                                @if(\Illuminate\Support\Facades\App::getLocale() == 'en')
                                    <li><a href="/{{\Illuminate\Support\Facades\App::getLocale()}}/pretraga/{{$subGroup->id}}">{{$subGroup->name_en}}</a>
                                        
                                    <ul>
                                        <a href="/{{\Illuminate\Support\Facades\App::getLocale()}}/napredna-pretraga?town=1&category={{$subGroup->id}}&nonstop=false&delivery=false&online=undefined&distSort=false&distFilter=500&lat=45.2671352&lng=19.8335496">
                                           Beograd
                                           </a>
                                ...
                                @else
                                    <li><a href="/{{\Illuminate\Support\Facades\App::getLocale()}}/pretraga/{{$subGroup->id}}">{{$subGroup->name_rs}}</a>
                                    <ul>
                                       <li>
                                           <a href="/{{\Illuminate\Support\Facades\App::getLocale()}}/napredna-pretraga?town=1&category={{$subGroup->id}}&nonstop=false&delivery=false&online=undefined&distSort=false&distFilter=500&lat=45.2671352&lng=19.8335496">
                                           Beograd
                                        ...
                                        <li><a href="/{{\Illuminate\Support\Facades\App::getLocale()}}/napredna-pretraga?town=2&category={{$subGroup->id}}&nonstop=false&delivery=false&online=undefined&distSort=false&distFilter=500&lat=45.2671352&lng=19.8335496">Novi Sad</a></li>
                                    </ul>
                            </li>
                                @endif
                            @endforeach
...
<div class="popup-wrapp" id="send-message" >
    ...
    <div class="popup-body">
        ...
        <div class="pull-left">
            <h4 id="nameOf"></h4>
            <span id="profesion">Vodoinstalater</span>
        </div>
        <form id="sendMessageForm" role="form" method="POST" data-url="{{ url('/send') }}">
            ...
            <div class="pull-left full-width form-group">
                ...
                <div class="form-group-results long">
                    <textarea class="form-control" name="message" id="message" placeholder="Unesite poruku..."></textarea>
                </div>
            </div>
            <div class="pull-right form-group bid-choose">
                <span class="accept" style="margin: 0;">
                    <input type="button" name="sendMessage" id="sendMessage" value="@lang('wy.send_message')" />
                </span>
            </div>
        </form>
    </div>
...
<script type="text/javascript" src="/js/jquery-ago.js"></script>
<script type="text/javascript">
    ...
    function populateTemplate(template, message, userId) {
        ...
        if(message.sender.is_readed == 0){
            template.find('.msg-holder').addClass(' new-msg');
        }
        
        if(message.sender.user_image !== null){
            template.find('.tabs-message-image img').attr('src', message.sender.user_image);   
        } else {
            template.find('.tabs-message-image img').attr('src', '../img/user/user-default.png'); 
        }
        //alert(Object.keys(message.sender));
        
        var link = '/{{\Illuminate\Support\Facades\App::getLocale()}}/profil/' + message.sender.id + '/' + message.sender.name;
        template.find('.go-to-profile').attr('href', link);
        ...
        template.find('.message-time abbr').attr('title', message.created_at);
